(() => {
	...
	const POST_MESSAGE_EVENT_TYPE = {
		...
		SlideUpdateUndoAndRedoStackLength:
			"SLIDE_UPDATE_UNDO_AND_REDO_STACK_LENGTH",
		...
		ChromeExtensionRequestPageAnalysis:
			"chrome_extension_request_page_analysis",
	...
	function getUniqueSelector(target) {
		...
		const pathString =
			pathParts.length > 0 ? pathParts.join(" → ") : "document";
...
`;
		...
		try {
			...
			elementsToClean.forEach((element, index) => {
				...
				if (element.hasAttribute("item-slide-selected")) {
					originalAttributes[index]["item-slide-selected"] =
						element.getAttribute("item-slide-selected");
					element.removeAttribute("item-slide-selected");
				}
				if (element.hasAttribute("data-slide-selected")) {
					originalAttributes[index]["data-slide-selected"] =
						element.getAttribute("data-slide-selected");
			...
			const fullPageCanvas = await Promise.race([
				capturePromise,
				timeoutPromise,
			]);
			...
			const croppedWidth = Math.max(
				1,
				(scaledCropWidth * devicePixelRatio) / 2,
			);
			const croppedHeight = Math.max(
				1,
				(scaledCropHeight * devicePixelRatio) / 2,
			);
	...
	function hanlderMessage(event) {
		...
		if (event.data.type === POST_MESSAGE_EVENT_TYPE.SlideSaveImageClicked) {
			const img = document.getElementById(EDIT_SLIDE_CONSTANT.CurClickEleId);
			if (img) {
				addUndoStack();
		...
		if (event.data.type === POST_MESSAGE_EVENT_TYPE.SlideCancelEdit) {
			...
			document
				.querySelectorAll("[data-slide-hover-selected]")
				.forEach((item) => {
					item.removeAttribute("data-slide-hover-selected");
				});
			document.getElementById(EDIT_SLIDE_CONSTANT.CurClickEleId);
			...
			if (dom) {
				...
				if (event.data.payload.attrs.style.textDecoration) {
					dom.style.textDecoration =
						event.data.payload.attrs.style.textDecoration;
				}
